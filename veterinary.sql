...
SELECT
    COUNT(A.appointid) AS Total_Appointments
FROM
    owners AS O
JOIN
    animals AS AN ON O.ownerid = AN.ownerid
JOIN
    appointments AS A ON AN.animalid = A.animalid
WHERE
    O.ofirstname = 'Maria';
    
SELECT
    AN.name,
    COUNT(A.appointid) AS Appointment_Count
FROM
    animals AS AN
JOIN
    appointments AS A ON AN.animalid = A.animalid
